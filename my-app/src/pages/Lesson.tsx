...
const Lesson: React.FC = () => {
    ...
    const fetchLesson = async () => {
      try {
        ...
        const lessonsRes = await fetch(
          import.meta.env.VITE_API_URL+`lessons?class_id=${data.class_id}&limit=100`,
          { credentials: "include" }
        );
        if (lessonsRes.ok) {
        ...
        const progressRes = await fetch(
          import.meta.env.VITE_API_URL+`lesson-progress?class_id=${data.class_id}`,
          {
            credentials: "include",
            headers: {
  ...
  const markAsCompleted = async () => {
    ...
    try {
      const token = getAuthToken();
      const res = await fetch(import.meta.env.VITE_API_URL+`lesson-progress/done`, {
        method: "POST",
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
  ...
  const unmarkAsCompleted = async () => {
    ...
    try {
      const token = getAuthToken();
      const res = await fetch(import.meta.env.VITE_API_URL+`lesson-progress/undone`, {
        method: "POST",
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
