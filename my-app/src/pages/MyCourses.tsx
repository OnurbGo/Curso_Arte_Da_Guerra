...
const MyCourses: React.FC = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get<{ id: number }>(
        import.meta.env.VITE_API_URL+`teachers/by-user/${decoded.user.id}`,
  ...
  const fetchLessons = (classId: number, page: number, limit: number) => {
    axios
      .get<{ data: Lesson[]; total: number }>(
        import.meta.env.VITE_API_URL+`lessons?class_id=${classId}&page=${page}&limit=${limit}`,
        { withCredentials: true }
      )
      .then((res) => {
  ...
  const handleCreateClass = () => {
    ...
    axios
      .post<Class>(import.meta.env.VITE_API_URL+`class`, payload, {
        withCredentials: true,
      })
      .then((res) => {
  ...
  const handleSubmitEditCourse = (courseId: number) => {
    ...
    axios
      .put<Class>(import.meta.env.VITE_API_URL+`class/${courseId}`, payload, {
        withCredentials: true,
      })
      .then((res) => {
        setClasses((prev) =>
  ...
  const handleDeleteCourse = (courseId: number) => {
    axios
      .delete(import.meta.env.VITE_API_URL+`class/${courseId}`, {
        withCredentials: true,
      })
      .then(() => {
  ...
  const handleCreateLesson = (
    ...
    axios
      .post<Lesson>(
        import.meta.env.VITE_API_URL+`class/${classId}/lessons`,
  ...
  const handleEditLesson = (updatedLesson: Lesson) => {
    axios
      .put<Lesson>(
        import.meta.env.VITE_API_URL+`lessons/${updatedLesson.id}`,
        updatedLesson,
        { withCredentials: true }
      )
      .then((res) => {
  ...
  const handleDeleteLesson = (lessonId: number, classId: number) => {
    axios
      .delete(import.meta.env.VITE_API_URL+`lessons/${lessonId}`, {
        withCredentials: true,
      })
      .then(() => {
