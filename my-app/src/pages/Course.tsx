...
const Course: React.FC = () => {
  ...
  const loadLessonsAndProgress = useCallback(async () => {
    try {
      const resL = await fetch(
        import.meta.env.VITE_API_URL+`lessons?class_id=${classId}&page=${currentPage}&limit=${lessonsPerPage}`,
        { credentials: "include" }
      );
      ...
      const resP = await fetch(
        import.meta.env.VITE_API_URL+`lesson-progress?class_id=${classId}`,
        {
          credentials: "include",
          headers: {
  ...
  useEffect(() => {
    const fetchInitial = async () => {
      try {
        const resC = await fetch(import.meta.env.VITE_API_URL+`class/${classId}`, {
          credentials: "include",
        });
        ...
        const resT = await fetch(
          import.meta.env.VITE_API_URL+`teachers/${dataC.master_id}`,
          {
            credentials: "include",
            headers: {
              "Content-Type": "application/json",
        ...
        const resU = await fetch(import.meta.env.VITE_API_URL+`users/${t.user_id}`, {
          credentials: "include",
        });
        if (resU.ok) {
        ...
        if (isAuthenticated && userId) {
          const resI = await fetch(
            import.meta.env.VITE_API_URL+`inscription?user_id=${userId}&class_id=${classId}`,
            {
              credentials: "include",
              headers: {
                "Content-Type": "application/json",
