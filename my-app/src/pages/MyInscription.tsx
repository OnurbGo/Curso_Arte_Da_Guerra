  ...
  const fetchProgressForCourse = useCallback(
    async (course: CourseDetails): Promise<CourseWithProgress> => {
      const lessonsRes = await fetch(
        import.meta.env.VITE_API_URL+`lessons?class_id=${course.id}&limit=100`,
        { credentials: "include" }
      );
      let totalLessons = 0;
      if (lessonsRes.ok) {
      ...
      const progressRes = await fetch(
        import.meta.env.VITE_API_URL+`lesson-progress?class_id=${course.id}`,
        {
          credentials: "include",
          headers: {
    ...
    const fetchCourses = async () => {
      ...
      try {
        ...
        const coursePromises = inscriptions.map((inscription) =>
          fetch(import.meta.env.VITE_API_URL+`class/${inscription.class_id}`, {
            credentials: "include",
          })
        );
